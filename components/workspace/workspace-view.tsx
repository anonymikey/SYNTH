...
export function WorkspaceView({ destination, onBackToAssistant }: { destination: WorkspaceDestination; onBackToAssistant: () => void }) {
  ...
  if (selectedModule) {
    return (
      <WorkspaceFrame icon={Icon} eyebrow="Roadmap preview" title={selectedModule.label} description={selectedModule.description} onBackToAssistant={onBackToAssistant}>
        <Card className="border-synth-violet/25 bg-synth-violet/5">
          <CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold">{selectedModule.label} is staged for the next phase</p>
              <p className="mt-1 text-xs leading-5 text-muted-foreground">
                This destination is interactive now so your workspace never dead-ends. Use the action below to return to the active Assistant workspace.
              </p>
            </div>
            <Button variant="outline" onClick={() => toast.success(`${selectedModule.label} roadmap saved to your workspace`)}>
              Save roadmap
            </Button>
          </CardContent>
        </Card>
        <WorkspaceActionGrid
          actions={["View module brief", "Open architecture notes", "Return to Assistant"]}
          onAction={(action) => action === "Return to Assistant" ? onBackToAssistant() : toast.info(`${action} is available as a SYNTH mock action`)}
        />
      </WorkspaceFrame>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row">
        <Input value={query} onChange={(event) => onQueryChange(event.target.value)} placeholder={`Search ${AREA_LABELS[area].toLowerCase()}...`} aria-label={`Search ${AREA_LABELS[area]}`} />
        <WorkspaceActionGrid actions={areaActions[area]} onAction={onAction} compact />
      </div>
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between gap-3">
            <CardTitle className="text-sm">{filtered.length} available</CardTitle>
        ...
        <CardContent className="space-y-2">
          {filtered.length ? (
            filtered.map((item) => (
              <div key={item} className="group flex items-center gap-3 rounded-lg border border-border bg-muted/20 p-3">
                <span className="size-2 rounded-full bg-synth-cyan/70" />
                <span className="min-w-0 flex-1 truncate text-sm">{item}</span>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => {
                    if (area === "history") setHistory(history.filter((entry) => entry !== item));
                    if (area === "skills") setSkills(skills.filter((entry) => entry !== item));
                    onAction(`${item} updated`);
                  }}
                >
                  {area === "history" ? "Delete" : area === "skills" ? "Disable" : "Open"}
                </Button>
              </div>
            ))
          ) : (
            <div className="rounded-lg border border-dashed border-border p-6 text-center text-sm text-muted-foreground">
              No matching items. Try a different search or create a new local item.
            </div>
          )}
        </CardContent>
      </Card>
      {area === "projects" && (
        <Button onClick={() => setProjectCreated(true)}>
          {projectCreated ? "Project draft created" : "Create project"}
        </Button>
      )}
    </div>
  );
...
function WorkspaceActionGrid({ actions, onAction, compact = false }: { actions: string[]; onAction: (action: string) => void; compact?: boolean }) {
  return (
    <div className={compact ? "flex flex-wrap gap-2" : "grid gap-3 sm:grid-cols-3"}>
      {actions.map((action) => (
        <Button key={action} variant="outline" className={compact ? "text-xs" : "h-auto justify-start py-3 text-left text-xs"} onClick={() => onAction(action)}>
          {action}
        </Button>
      ))}
    </div>
