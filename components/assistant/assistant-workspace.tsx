...
export function AssistantWorkspace({ project }: { project: ProjectSummary }) {
  ...
  const chat = useAssistantChat({ project, context: DEFAULT_CONTEXT, modelId, agentMode });
  const showWelcome = chat.messages.length === 0;
  const activeModel = useMemo(() => mockModels.find((model) => model.id === modelId), [modelId]);
